<template>
    <div class="data-model wrap">
        <div class="table-head">
            ...
            <a-button
                    @click="$router.push({name: 'newDataset'})"
                    icon="plus"
                    type="primary"
            >新增
            </a-button>
        ...
        <a-table :columns="columns" :dataSource="data" rowKey="ID">
            ...
            <template slot="status" slot-scope="status">
                <span>
                    <a-tag :color="datasetStatusTagColor(status)">{{datasetStatusMap(status)}}</a-tag>
...
<script lang="ts">
    ...
    import {deleteDataset, getDataset, getDatasetDownloadUrl, getDatasetList, IDatasetListItem} from '@/api/dataset'
    ...
    export default class DatasetList extends Vue {
        ...
        private datasetStatusMap(status: number): string {
            return ['创建中', '不可用', '可用', '被占用'][status]
        }

        private datasetStatusTagColor(status: number): string {
            return ['orange', 'red', 'green', 'cyan'][status]
        }

        private sizeMap(size: number): string {
            const index = Math.floor(Math.log(size) / Math.log(1024))
            return (size / Math.pow(1024, index)).toFixed(1) + ['B', 'KB', 'MB', 'GB'][index]
        }

        private _delete(datasetID: number): void {
        ...
        private download(dataset: IDatasetListItem): void {
            const link = document.createElement('a')
            link.href = getDatasetDownloadUrl(dataset.ID)
        ...
        private created() {
            getDatasetList().then((res: any): void => {
                this.data = res.data.map((item: IDatasetListItem) => {
            ...
            this.queryStatus = setInterval(() => {
                let unreadyList = this.data.filter((item) => {
                    return item.PVCStatus === 0 || item.Status === 0
                })
                ...
                unreadyList.forEach((item) => {
                    getDataset(item.ID).then((response) => {
                        [item.PVCStatus, item.Status] =
                            [response.data.PVCStatus, response.data.Status]
                    })
                })
            }, 2000)
        }
