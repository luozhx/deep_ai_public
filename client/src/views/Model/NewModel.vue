<template>
  <div class="model-develop wrap">
    <div class="new-head">
      <h2>创建模型</h2>
      <a-button type="danger" @click="$router.push({name: 'modelList'})"> <a-icon type="left" />返回列表 </a-button>
    </div>
    <a-form :form="form" @submit="handleSubmit">
        ...
        <a-form-item :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }" label="网络类定义文件" v-if="!isFrameworkTensorFlow">
            <a-textarea :autosize="{minRows: 20}"
                        v-decorator="['Network', {rules: [{required: true, message: '请填写网络类定义代码文件'}]}]"></a-textarea>
            <span style="color: #9c9c9c">网络类定义代码将会保存为名为 nn_model.py 文件（请注意import相关变量，此文件将在推理服务中使用，请正确填写）</span>
        </a-form-item>
...
<script lang="ts">
    ...
    import {createNewModel} from '@/api/model'
        ...
        private handleSubmit(e: any) {
            ...
            this.form.validateFields((err: any, values: any) => {
                if (!err) {
                    ...
                    createNewModel(values).then((response) => {
                        this.$message.success('创建模型成功')
                        this.$router.replace({
                            name: 'modelList'
                        })
                    }).catch(() => {
                        this.$message.error('创建模型失败，请重试')
                    }).finally(() => {
